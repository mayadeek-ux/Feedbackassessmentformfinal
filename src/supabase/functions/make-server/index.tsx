...
app.post("/assignments", async (c) => {
  ...
  try {
    const { event_id, assessor_id, candidate_id, group_id, type, case_study, due_date } = await c.req.json();
...
app.get("/events/:eventId/assessor/:assessorId/assignments", async (c) => {
  ...
  try {
    ...
    // Ensure user can only access their own assignments unless admin
...
app.post("/assessments", async (c) => {
  ...
  try {
    const { assignment_id, scores, notes, total_score, performance_band, submitted } = await c.req.json();
    ...
    if (user.id !== assignment.assessor_id) {
      return c.json({ error: 'Access denied' }, 403);
    }
    ...
    const newStatus = submitted ? 'submitted' : (assignment.status === 'not_started' ? 'in_progress' : assignment.status);
...
app.post("/groups", async (c) => {
  ...
  try {
    ...
    // Update candidates with group_id
...
// Initialize demo data - NOT NEEDED with SQL migrations, but kept for compatibility
app.post("/init-demo", async (c) => {
  ...
  try {
    ...
    // Demo data is created by SQL migrations
    // Just create demo candidates here
